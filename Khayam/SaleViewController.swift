...
class SaleViewController: UIViewController {
    ...
    var saleImageArray = [ UIImage.init(named: "happyBD"), UIImage(named: "uberDel"), UIImage(named: "lunch99")]
   ...
   var timer = Timer()
        ...
        override func viewDidLoad() {
            super.viewDidLoad()
            pageView.numberOfPages = saleImageArray.count
       ...
       @objc func changeImage() {
        ...
        if counter < saleImageArray.count {
    ...
    extension SaleViewController: UICollectionViewDelegate, UICollectionViewDataSource {
        func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
            return saleImageArray.count
        }
        
        func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath)
            if let vc = cell.viewWithTag(111) as? UIImageView {
                vc.image = saleImageArray[indexPath.row]
            }
            return cell
        }
    }
